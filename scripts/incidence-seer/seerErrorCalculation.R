...
fileToLoad <- '2021-08-20-SEER-2010-to-2013-pop-2010-rates.RData'
load(file.path('..', '..', 'outputs', 'seer', 'count-data', fileToLoad))
source("../util/seerInfo.R");

outputPath <- "../../outputs/seer/count-data/"
plotPath <- "../../outputs/seer/count-data/error-plots/" 

firstyr <- strsplit(fileToLoad, '-')[[1]][5]
lastyr <- strsplit(fileToLoad, '-')[[1]][7]
...
idxNorm <- which(sumcounts$Count > 10)
